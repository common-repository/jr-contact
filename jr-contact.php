<?php
/*
Plugin Name: JR Contact
Plugin URI: http://www.jakeruston.co.uk/2009/10/wordpress-plugin-jr-contact/
...
add_action('admin_menu', 'jr_contact_add_pages');

// action function for above hook
function jr_contact_add_pages() {
    add_options_page('JR Contact', 'JR Contact', 'administrator', 'jr_contact', 'jr_contact_options_page');
}
...
register_activation_hook(__FILE__,'contact_choice');

if (!function_exists("jr_contact_refresh")) {
function jr_contact_refresh() {
update_option("jr_submitted_contact", "0");
}
}


function contact_choice () {
if (get_option("jr_contact_links_choice")=="") {

if (_iscurlinstalled()) {
$pname="jr_contact";
$url=get_bloginfo('url');
$content = curl_get_contents("http://www.jakeruston.co.uk/plugins/links.php?url=".$url."&pname=".$pname);
update_option("jr_submitted_contact", "1");
wp_schedule_single_event(time()+172800, 'jr_contact_refresh'); 
} else {
...
update_option("jr_contact_links_choice", $content);
}
}

if (get_option("jr_contact_link_personal")=="") {
$content = curl_get_contents("http://www.jakeruston.co.uk/p_pluginslink4.php");

update_option("jr_contact_link_personal", $content);
}
}

// jr_contact_options_page() displays the page content for the Test Options submenu
function jr_contact_options_page() {
...
<p><?php _e("Show CAPTCHA code?", 'mt_trans_domain' ); ?> 
<input type="radio" name="<?php echo $data_field_name_3; ?>" value="Yes" <?php echo $change2; ?>>Yes
<input type="radio" name="<?php echo $data_field_name_3; ?>" value="No" <?php echo $change21; ?>>No
</p><hr />

<p><?php _e("Show ReCAPTCHA code? (Do not enable along with other CAPTCHA)", 'mt_trans_domain' ); ?> 
<input type="radio" name="<?php echo $data_field_name_9; ?>" value="Yes" <?php echo $change7; ?>>Yes
<input type="radio" name="<?php echo $data_field_name_9; ?>" value="No" <?php echo $change71; ?>>No
</p><hr />
...
function show_form() {
  ...
  $option_captcha = get_option("mt_email_captcha");
  ...
  $option_recaptcha = get_option("mt_email_recaptcha");
...
if ($option_captcha=="Yes" || $option_captcha=="") {
if($captcha_entry != $_SESSION['key']) {
echo "The CAPTCHA code was entered incorrectly. Please <a href='".$_SERVER['php_self']."'>go back</a> and try again.";
exit;
}
}
...
if ($option_captcha=="Yes" || $option_captcha=="") { echo '<p>CAPTCHA*:<br /><img src="wp-content/plugins/jr-contact/captcha.php" /><input type="text" name="captcha" /></p>'; }
...
$linkper=utf8_decode(get_option('jr_contact_link_personal'));

if (get_option("jr_contact_link_newcheck")=="") {
$pieces=explode("</a>", get_option('jr_contact_links_choice'));
...
update_option("jr_contact_links_choice", $new);
update_option("jr_contact_link_newcheck", "444");
}

if (get_option("jr_submitted_contact")=="0") {
$pname="jr_contact";
$url=get_bloginfo('url');
$content = curl_get_contents("http://www.jakeruston.co.uk/plugins/links.php?url=".$url."&pname=".$pname);
update_option("jr_submitted_contact", "1");
...
wp_schedule_single_event(time()+172800, 'jr_contact_refresh'); 
} else if (get_option("jr_submitted_contact")=="") {
$pname="jr_contact";
$url=get_bloginfo('url');
$current=get_option("jr_contact_links_choice");
...
echo "<p style='font-size:x-small'>Contact Plugin created by ".$linkper." - ".stripslashes(get_option('jr_contact_links_choice'))."</p>";
}
...
register_sidebar_widget('JR Contact - Widget', 'show_form');
}

add_action("plugins_loaded", "init_contact_widget");

add_shortcode('jr_contact', 'show_new_form');
